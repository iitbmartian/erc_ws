...
def launch_setup(context, *args, **kwargs):      
    ...
    return [
        ...
        #                'zedx_center_optical_frame', 'panther/base_link/zedx_stereolabs_zed_color'],
        ...
        # Node(
        #     package='image_proc',
        #     executable='image_proc_node',  # Changed from 'image_proc' to 'image_proc_node'
        #     name='image_proc',
        #     namespace='zedx/zed_node',
        #     parameters=[{
        #         'use_sim_time': True,
        #         'queue_size': 10
        #     }]
        # ),
        # Static transform for depth image frame (if needed)
        # Node(
        #     package='tf2_ros',
        #     executable='static_transform_publisher',
        #     name='static_tf_depth',
        #     arguments=['0', '0', '0', '0', '0', '0', 
        #                'zedx_center_optical_frame', 'panther/base_link/zedx_stereolabs_zed_depth']
        # ),
        # Node(            
        #     package='tf2_ros',
        #     executable='static_transform_publisher',
        #     name='static_tf_depth',
        #     arguments=['0', '0', '0', '0', '0', '0',
        #                'base_link', 'panther/base_link/velodyne_puck_sensor']
        #      ),
        ...
        Node(
            ...
            parameters=[{
                ...
                'Rtabmap/DetectionRate': '100.0',  # Set the detection rate (in Hz)
            }],
            remappings=[
...
def generate_launch_description():
    ...
    config_rtabmap = os.path.join(
        get_package_share_directory('rtabmap'), 'config', 'rtabmap.ini'
    )
    ...
    return LaunchDescription([
        ...
        # Simulation Params - Sync clocks
        DeclareLaunchArgument('sim', default_value='true', description='Flag to enable simulation (sets use_sim_time to true if sim is true)'),
        DeclareLaunchArgument('use_sim_time', default_value=LaunchConfiguration('sim') , description='Use simulation (Gazebo) clock if true'),
        ...
        DeclareLaunchArgument('frame_id',       default_value='panther/base_link',          description='Fixed frame id of the robot (base frame), you may set "base_link" or "base_footprint" if they are published. For camera-only config, this could be "camera_link".'),
        DeclareLaunchArgument('odom_frame_id',  default_value='panther/odom',                   description='If set, TF is used to get odometry instead of the topic.'),
        DeclareLaunchArgument('map_frame_id',   default_value='map',                description='Output map frame id (TF).'),
        DeclareLaunchArgument('publish_tf_map', default_value='true',               description='Publish TF between map and odomerty.'),        DeclareLaunchArgument('map_topic',      default_value='map',                description='Map topic name.'),
        ...
        DeclareLaunchArgument('rgb_topic',           default_value='/panther/camera_front/image_raw',       description=''),
        DeclareLaunchArgument('depth_topic',         default_value='/zedx/zed_node/depth', description=''),
        DeclareLaunchArgument('camera_info_topic',   default_value='/panther/camera_front/camera_info',            description=''),
        ...
        DeclareLaunchArgument('subscribe_scan_cloud', default_value='true',       description='Enable PointCloud subscription.'),
        DeclareLaunchArgument('scan_cloud_topic',     default_value='/panther/cx/lslidar_point_cloud', description='PointCloud topic name.'),
        ...
        DeclareLaunchArgument('odom_topic',                 default_value='/panther/odometry/filtered',  description='Odometry topic name.'),
        DeclareLaunchArgument('vo_frame_id',                default_value= 'panther/odom', description='Visual/Icp odometry frame ID for TF.'),
        DeclareLaunchArgument('publish_tf_odom',            default_value='false',  description=''),DeclareLaunchArgument('odom_tf_angular_variance',   default_value='0.01',    description='If TF is used to get odometry, this is the default angular variance'),
